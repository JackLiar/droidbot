...
class ADB(Adapter):
    ...
    def run_cmd(self, extra_args: Union[str, List[str]]) -> str:
        ...
        if not isinstance(extra_args, list):
            msg = f"invalid arguments: {extra_args}\nshould be list or str, {type(extra_args)} given"
    ...
    def shell(self, extra_args: Union[str, List[str]]) -> str:
        """
    ...
    def get_ro_debuggable(self) -> int:
        """
    ...
    def get_display_info(self):
        ...
        """
        display_info = {}
        logical_display_re = re.compile(
            ".*DisplayViewport{valid=true, .*orientation=(?P<orientation>\d+),"
    ...
    def __transform_point_by_orientation(self, xy: Tuple[int, int], orientation_orig: int, orientation_dest: int):
        (x, y) = xy
    ...
    def touch(self, x: int, y: int, orientation: int = -1, event_type: EventType = EventType.DOWN_AND_UP):
        if orientation == -1:
            orientation = self.get_orientation()
        self.shell(
            "input tap %d %d" % self.__transform_point_by_orientation((x, y), orientation, self.get_orientation())
        )
    ...
    def drag(self, start_xy: Tuple[int, int], end_xy: Tuple[int, int], duration: int, orientation: int = -1):
        """
        ...
        if version <= 15:
            ...
            self.shell(f"input swipe {x0} {y0} {x1} {y1}")
        else:
            self.shell(f"input touchscreen swipe {x0} {y0} {x1} {y1} {duration}")

    def type(self, text: Union[str, Any]):
